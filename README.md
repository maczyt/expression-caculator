...
> 💡 这里不会再把三地址转成机器码执行，我们通过eval来执行即可

> 2020-04-24 更新

上面我们通过消除左递归来处理表达式，但是我们并没有保证结合性，也就会导致 `5 - 3 - 1` 被计算成 `3`，因为先执行了 `3-1`, 才执行 `5-2`。

因为我们处理的表达式是中缀的，这个是给我们看的，给计算机的话，咱们需要转成后缀表达式。(也就是前面的要被推翻了，😭)
...
![image-20200424224739652](./image/img8.png)
