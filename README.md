...
到了这步，我们距离成功已经近了一半，已经把`字符流`->`符号流`->`AST`

首先我们引入一个概念：`SDD`(Syntax Directed Definition)：
...
我们采用三地址运算的方式，简单理解就是每个表达式最多只有三个地址。如：

- source: ` " 49 * 63 / 58 * 36 "`
- AST:![image-20200423203434897](./image/img6.png)
- 三地址表示: ![image-20200423203528616](./image/img7.png)



可以看出来最终的三地址表示，我们只需要一个个计算就可以得到最终的`p2`，也就是`source`的结果。
...
SDD的文法和语法分析的产生式有什么区别？

> SDD 自底向上，语法分析自顶向下？

三地址：地址和偏移量(offset)

`符号 -> ASTNode -> 地址（三地址代码）-> 操作符（运行）`
